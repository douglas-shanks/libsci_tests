program complex_matrix_solve
    ...
    complex(kind=8), allocatable :: A(:,:), As(:,:)
    complex(kind=8), allocatable :: b(:), x(:), y(:), diff(:)
    ...
    call zgesv(N, 1, A, lda, pivot, b, ldb, info)
    ...
    call zgemv('N', N, N, (1.0_8, 0.0_8), As, lda, b, 1, (0.0_8, 0.0_8), y, 1)
    ...
    !print *, "Norm check (should be zero):"
    !print*, norm

    deallocate(A, b, x, y, pivot)

end program complex_matrix_solve
